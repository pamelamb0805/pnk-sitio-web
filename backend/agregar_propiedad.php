...
document.getElementById('frm_prop').addEventListener('submit', function(e) {
    if (document.getElementById('accion').value !== 'guardar') return;

    var errores = [];
    if (!document.getElementById('frm_tipo').value) errores.push('Seleccione el tipo de propiedad.');
    if (!document.getElementById('sel_region').value) errores.push('Seleccione una región.');
    if (!document.getElementById('sel_provincia').value) errores.push('Seleccione una provincia.');
    if (!document.getElementById('sel_comuna').value) errores.push('Seleccione una comuna.');

    var fotos = document.getElementById('frm_fotos').files;
    if (fotos.length > 10) errores.push('Máximo 10 fotografías permitidas.');

    if (errores.length > 0) {
        e.preventDefault();
        if (typeof Swal !== 'undefined') {
            Swal.fire({ icon: 'warning', title: 'Campos incompletos', html: errores.map(m => '• ' + m).join('<br>'), confirmButtonColor: '#b0a78f' });
        } else {
            alert(errores.join('\n'));
        }
    }
